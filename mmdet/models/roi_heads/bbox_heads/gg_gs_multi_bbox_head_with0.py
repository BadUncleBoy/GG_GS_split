...
class GGNN(nn.Module):
    ...
    def __init__(self, A, fc_out_channels, init_weights, state_dim=10, n_steps=5):
        ...
        for _ in A:
            ...
            self.state2feature_dim_list.append(
                nn.Sequential(
    ...
    def forward(self, feat):
        ...
        for index, a in enumerate(self.A):
            state = self.feature2state_dim_list[index](self.classifier_weight)
            for _ in range(self.n_steps):
                state = self.propogator_list[index](state, a)
            result = self.state2feature_dim_list[index](state)
            gg_feats += torch.mm(feat, result.permute(1,0)) * self.feat_weight[index]
